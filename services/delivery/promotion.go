...
func DecidePromotion(env *delivery_model.Environment, state PredecessorState, canBypass bool) Decision {
	d := Decision{Outcome: OutcomeProceed, PredecessorState: state}
	if env == nil || state != PredecessorNever {
		return d
	}
	predecessor := delivery_model.NormalizeEnvironmentName(env.Predecessor)
	name := delivery_model.NormalizeEnvironmentName(env.Name)
...
type ErrPromotionNotFound struct{ Err *delivery_model.Error }
...
func PlanPromotion(ctx reqctx.RequestContext, req PromotionRequest) (*Promotion, error) {
	...
	environment := delivery_model.NormalizeEnvironmentName(req.Environment)
	if environment == "" {
		return nil, &delivery_model.Error{
	...
	env, err := delivery_model.GetEnvironment(ctx, req.Repo.ID, environment)
	if err != nil {
		if hubErr, ok := err.(*delivery_model.Error); ok {
	...
	if err != nil {
		return nil, &ErrPromotionNotFound{Err: &delivery_model.Error{
...
func dispatchFailed(ctx reqctx.RequestContext, req PromotionRequest, out *Promotion, message, action string) error {
	if err := appendPromotionEvent(ctx, req, out, delivery_model.AuditFailed, ""); err != nil {
		log.Error("delivery: record failed dispatch of %s to %s: %v", out.ReleaseTag, out.Environment, err)
	}
	return &delivery_model.Error{Message: message, SuggestedAction: action}
}
...
func appendPromotionEvent(ctx reqctx.RequestContext, req PromotionRequest, out *Promotion, event, reason string) error {
	...
	return delivery_model.AppendAuditEvent(ctx, &delivery_model.AuditEvent{
		Event:        event,
...
func promotionEvents(ctx reqctx.RequestContext, repoID int64, environment, predecessor, releaseTag string) ([]Event, error) {
	...
	if predecessor = delivery_model.NormalizeEnvironmentName(predecessor); predecessor != "" {
...
func liveRelease(ctx reqctx.RequestContext, repoID int64, environment string) string {
	rows, err := delivery_model.FindAuditEvents(ctx,
		builder.Eq{"repo_id": repoID, "environment": environment, "event": delivery_model.AuditSucceeded},
