...
func runAt(id, repoID int64, workflow string, status actions_model.Status, offset, duration int64) delivery_model.RunFact {
	created := day + offset
	fact := delivery_model.RunFact{
...
func TestDeliveryDailyTrendIncludesQuietDaysAndDeployments(t *testing.T) {
	...
	deployments := []delivery_model.DeploymentFact{
...
func TestDeliveryBuildTrendsMatchesTheDeliveryTables(t *testing.T) {
	...
	require.NoError(t, delivery_model.AppendDeployment(ctx, &delivery_model.Deployment{
