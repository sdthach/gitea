...
func SummarizeRuns(facts []delivery_model.RunFact, window Window, accessibleRepos int, disabled map[int64][]string) Summary {
...
func (c *counter) add(f delivery_model.RunFact) {
	c.runs++
...
func AggregateRepos(facts []delivery_model.RunFact, names map[int64]string) []RepoStat {
...
func AggregateWorkflows(facts []delivery_model.RunFact, names map[int64]string, disabled map[int64][]string) []WorkflowStat {
	type wfKey struct {
		repoID     int64
		workflowID string
	}
...
func BuildOverview(ctx context.Context, opts OverviewOptions) (*Overview, error) {
	...
	disabled, err := delivery_model.FindDisabledWorkflows(ctx, repoIDs)
	if err != nil {
		return nil, err
	}
	current, truncatedNow, err := delivery_model.FindRunFacts(ctx, repoIDs, opts.Window.FromUnix, opts.Window.ToUnix)
	if err != nil {
		return nil, err
	}
	previousWindow := opts.Window.Previous()
	previous, truncatedBefore, err := delivery_model.FindRunFacts(ctx, repoIDs, previousWindow.FromUnix, previousWindow.ToUnix)
	if err != nil {
		return nil, err
	}
	return &Overview{
...
func BuildTrends(ctx context.Context, opts OverviewOptions) ([]TrendPoint, bool, error) {
	...
	facts, truncated, err := delivery_model.FindRunFacts(ctx, repoIDs, opts.Window.FromUnix, opts.Window.ToUnix)
	if err != nil {
		return nil, false, err
	}
	deployments, err := delivery_model.FindDeploymentFacts(ctx, repoIDs, opts.Window.FromUnix, opts.Window.ToUnix)
	if err != nil {
		return nil, false, err
	}
