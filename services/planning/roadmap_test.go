...
func managed(in BarInput) BarInput {
	in.IssueID, in.Number, in.Title, in.ParentIssueID = 9001, 1, "task", 42
...
// An issue with no bar is listed with the reason, never given one.
func TestPlanningUnmanagedIssueGetsNoBarAndOneStatedReason(t *testing.T) {
	...
	assert.Contains(t, listed.Reason, "no type, no parent and no start date")
...
// A parent or milestone row spans earliest start to latest end of its children, and its
// progress is the same close percentage every other view uses.
func TestPlanningBuildRollupCoversItsChildrenAndUsesTheSharedProgressDefinition(t *testing.T) {
	bars := []Bar{
	...
	assert.Equal(t, 33, row.Progress, "closed over total, and no second definition")
	assert.True(t, row.EndInferred, "a rollup is never firmer than the bars it is made of")

	_, ok = BuildRollup("parent", "empty", "empty", nil)
...
func TestPlanningBuildRollupsEmitsParentsThenMilestones(t *testing.T) {
	bars := []Bar{
		...
		{IssueID: 2, ParentIssueID: 200, StartUnix: 150, EndUnix: 900, IsClosed: true},
		{IssueID: 3, StartUnix: 50, EndUnix: 60},
	}
	rows := BuildRollups(bars)
	require.Len(t, rows, 3)
	assert.Equal(t, []string{"parent", "parent", "milestone"}, []string{rows[0].Kind, rows[1].Kind, rows[2].Kind})
	assert.Equal(t, "100", rows[0].Key, "parents order by issue id")
	assert.Equal(t, "200", rows[1].Key)
	assert.Equal(t, "beta", rows[2].Label)
	assert.Equal(t, 0, rows[0].Progress)
	assert.Equal(t, 100, rows[1].Progress)
}

// TestPlanningBuildRollupsExcludesTheParentFromItsOwnRollup is what makes the containment
// check meaningful: a bar's ParentIssueID always points at ITS OWN parent, never at itself, so
// the parent's own bar is never counted among its own children.
func TestPlanningBuildRollupsExcludesTheParentFromItsOwnRollup(t *testing.T) {
	bars := []Bar{
		{IssueID: 42, Number: 42, Title: "checkout", Type: "epic", StartUnix: 100, EndUnix: 5000},
		{IssueID: 1, Number: 57, ParentIssueID: 42, Type: "story", StartUnix: 300, EndUnix: 900, IsClosed: true},
		{IssueID: 2, Number: 58, ParentIssueID: 42, Type: "task", StartUnix: 200, EndUnix: 400},
	}
	rows := BuildRollups(bars)
	require.Len(t, rows, 1)
	assert.Equal(t, 2, rows[0].Children, "the parent issue is not one of its own children")
	...
	assert.EqualValues(t, 42, rows[0].IssueID, "the row names the parent issue, so a bracket can be opened")
	assert.Equal(t, "checkout", rows[0].Label, "the row is labelled with the parent's own title")
	assert.Equal(t, "epic", rows[0].Type)

	// The same parent issue filed under a milestone is genuinely one of that milestone's
...
// TestPlanningContainmentFlagsAParentThatEndsBeforeItsChildren is the warning the chart is the
// only place to see, and the case it must NOT fire on: children running in parallel past no
// deadline at all.
func TestPlanningContainmentFlagsAParentThatEndsBeforeItsChildren(t *testing.T) {
	// 2026-03-11 declared, 2026-03-25 derived: a fortnight of overhang.
	declared := Bar{IssueID: 42, Number: 42, Title: "checkout", Type: "epic", StartUnix: 1772323200, EndUnix: 1773187200}
	child := Bar{IssueID: 7, Number: 57, ParentIssueID: 42, Type: "story", StartUnix: 1772323200, EndUnix: 1774396800}
	...
	assert.Equal(t, `parent "checkout" (#42) ends 14 days before the work filed under it`, row.Warning)
	assert.Equal(t, "Move the parent's deadline to 2026-03-25, or move story #57 earlier.", row.SuggestedAction)
	...
	// Work that starts before the parent it belongs to is the other half of containment.
	declared.StartUnix = 1772928000 // 2026-03-08
	rows = BuildRollups([]Bar{declared, child})
	require.Len(t, rows, 1)
	assert.False(t, rows[0].ContainsChildren)
	assert.Equal(t, `parent "checkout" (#42) starts 7 days after the work filed under it`, rows[0].Warning)
	assert.Contains(t, rows[0].SuggestedAction, "Move the parent's start to 2026-03-01")
}
...
func TestPlanningRollupRowPartialWithdrawsItsProgress(t *testing.T) {
	row, ok := BuildRollup("parent", "checkout", "checkout", []Bar{
...
// TestPlanningResolveBarUsesTheAssignedType: the chart cannot tell a story from a bug without
// it. The type comes from the issue's own assignment, not a label.
func TestPlanningResolveBarUsesTheAssignedType(t *testing.T) {
	...
	assert.Equal(t, "story", GroupKeyFor(GroupInput{TypeName: bar.Type, Assignees: bar.Assignees}, GroupType),
...
// schedule, or a recorded parent, each alone is enough to draw a bar.
func TestPlanningManagedRespectsEveryDeclaredCondition(t *testing.T) {
	...
	assert.True(t, Managed(BarInput{ParentIssueID: 42}))
...
func TestPlanningParseZoomAcceptsTheDeclaredSetAndRefusesTheRest(t *testing.T) {
	for _, tc := range []struct {
		...
		{"", ZoomIssue}, {"issue", ZoomIssue}, {"parent", ZoomParent}, {" MILESTONE ", ZoomMilestone},
	} {
		zoom, ok := ParseZoom(tc.raw)
		require.True(t, ok, tc.raw)
		assert.Equal(t, tc.want, zoom, tc.raw)
	}
	zoom, ok := ParseZoom("epic")
	assert.False(t, ok, "the retired epic zoom is refused, not silently accepted")
...
// A parent that is nobody's parent (no bar carries its id) forms no rollup: there is nothing
// to roll up.
func TestPlanningBuildRollupsSkipsAParentWithNoChildren(t *testing.T) {
	rows := BuildRollups([]Bar{{IssueID: 42, Number: 42, Title: "lonely", Type: "epic", StartUnix: 100, EndUnix: 900}})
	assert.Empty(t, rows)
}
