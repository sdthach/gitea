...
// TestInsightsRunStateOfCoversEveryActionsStatus is the mapping the tiles group on. Every
// upstream status has to land in a named bucket, or a run would vanish from a total without
// anything saying so.
func TestInsightsRunStateOfCoversEveryActionsStatus(t *testing.T) {
...
// TestInsightsRunStatusCodesWidensAMultiStatusState is why the /runs status filter renders as
// an IN: in_progress covers two upstream statuses, and matching only one would be wrong.
func TestInsightsRunStatusCodesWidensAMultiStatusState(t *testing.T) {
...
func TestInsightsWindowIsHalfOpenAndItsPredecessorAdjoinsIt(t *testing.T) {
...
// TestInsightsSummarizeRunsCountsEveryState covers the tile set over a fixture run set.
func TestInsightsSummarizeRunsCountsEveryState(t *testing.T) {
...
func TestInsightsSummarizeRunsOnAnEmptyWindow(t *testing.T) {
...
func TestInsightsAggregateReposRanksByVolumeAndCarriesTheRate(t *testing.T) {
	facts := []deployments_model.RunFact{
		runAt(1, 10, "ci.yaml", actions_model.StatusSuccess, 10, 30),
...
func TestInsightsAggregateWorkflowsMarksTheDisabledOnes(t *testing.T) {
	facts := []deployments_model.RunFact{
		runAt(1, 10, "ci.yaml", actions_model.StatusSuccess, 10, 30),
...
// TestInsightsDailyTrendIncludesQuietDaysAndDeployments: the deployment count comes
// from the fork's own table, so this dashboard and the deployments matrix cannot disagree.
func TestInsightsDailyTrendIncludesQuietDaysAndDeployments(t *testing.T) {
...
func TestInsightsDailyTrendBucketsPreEpochTimestampsIntoTheirOwnDay(t *testing.T) {
...
// TestInsightsOptionsScopeIsFailClosed is the security branch. A filter that widened
// instead of narrowing here would leak every repository's runs to any caller.
func TestInsightsOptionsScopeIsFailClosed(t *testing.T) {
	assert.Nil(t, InsightsOptions{RepoIDs: nil}.scope(),
		"no accessible repository aggregates nothing, never everything")
	assert.Equal(t, []int64{1, 2}, InsightsOptions{RepoIDs: []int64{1, 2}}.scope())
	assert.Equal(t, []int64{2}, InsightsOptions{RepoIDs: []int64{1, 2}, RepoID: 2}.scope())
	assert.Nil(t, InsightsOptions{RepoIDs: []int64{1, 2}, RepoID: 99}.scope(),
		"asking for a repository outside the accessible set narrows to nothing, it does not widen")
}

// TestInsightsBuildExcludesARepositoryTheViewerCannotSee exercises the exported entry
// point the /overview handler calls — not the projection underneath it — so a wrapper that
// dropped the scope could not pass.
func TestInsightsBuildExcludesARepositoryTheViewerCannotSee(t *testing.T) {
	...
	both, err := BuildInsights(ctx, InsightsOptions{RepoIDs: []int64{publicRepo, privateRepo}, Window: window})
	require.NoError(t, err)
	require.Positive(t, both.Summary.TotalRuns)

	onlyPublic, err := BuildInsights(ctx, InsightsOptions{RepoIDs: []int64{publicRepo}, Window: window})
	require.NoError(t, err)
	...
	none, err := BuildInsights(ctx, InsightsOptions{RepoIDs: nil, Window: window})
	require.NoError(t, err)
	assert.Equal(t, int64(0), none.Summary.TotalRuns)
	assert.Equal(t, int64(0), none.Summary.InactiveRepositories)
}

// TestInsightsBuildCountsMatchThePerRepositoryQuery: the aggregate has to
// agree with the same question asked one repository at a time.
func TestInsightsBuildCountsMatchThePerRepositoryQuery(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())
	ctx := t.Context()
	...
	aggregate, err := BuildInsights(ctx, InsightsOptions{RepoIDs: repoIDs, Window: window})
	require.NoError(t, err)

	var perRepo int64
	perRepoStates := map[string]int64{}
	for _, id := range repoIDs {
		one, err := BuildInsights(ctx, InsightsOptions{RepoIDs: repoIDs, RepoID: id, Window: window})
		require.NoError(t, err)
...
// TestInsightsBuildTrendsMatchesTheDeploymentTables is the deployment half: the trend's
// deployment count is the fork's own table, read back.
func TestInsightsBuildTrendsMatchesTheDeploymentTables(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())
	ctx := t.Context()
	...
	points, _, err := BuildTrends(ctx, InsightsOptions{RepoIDs: []int64{4}, Window: window})
	require.NoError(t, err)
	...
	assert.Equal(t, int64(len(rows)), deployments,
		"the trend's deployment count is the deployment table, so both dashboards share one source of truth")
	assert.Equal(t, int64(2), deployments)

	elsewhere, _, err := BuildTrends(ctx, InsightsOptions{RepoIDs: []int64{2}, Window: window})
	require.NoError(t, err)
	var leaked int64
...
// TestInsightsBuildRepoStatsAndWorkflowStatsAreScoped exercises the two remaining exported
// entry points the handlers call.
func TestInsightsBuildRepoStatsAndWorkflowStatsAreScoped(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())
	ctx := t.Context()
	window := Window{FromUnix: 0, ToUnix: 1 << 40, Days: 1}

	repos, _, err := BuildRepoStats(ctx, InsightsOptions{RepoIDs: []int64{4}, Window: window})
	require.NoError(t, err)
	...
	workflows, _, err := BuildWorkflowStats(ctx, InsightsOptions{RepoIDs: []int64{4}, Window: window})
	require.NoError(t, err)
	...
	empty, _, err := BuildRepoStats(ctx, InsightsOptions{RepoIDs: nil, Window: window})
	require.NoError(t, err)
	assert.Empty(t, empty)

	noWorkflows, _, err := BuildWorkflowStats(ctx, InsightsOptions{RepoIDs: nil, Window: window})
	require.NoError(t, err)
	assert.Empty(t, noWorkflows)
}

// TestInsightsSortWorkflowStatsOrdersAndTieBreaks covers the sort on its own, with an input
...
func TestInsightsSortWorkflowStatsOrdersAndTieBreaks(t *testing.T) {
	rows := []WorkflowStat{
...
// TestInsightsSortRepoStatsOrdersAndTieBreaks is the same for the repository rows.
func TestInsightsSortRepoStatsOrdersAndTieBreaks(t *testing.T) {
	rows := []RepoStat{
