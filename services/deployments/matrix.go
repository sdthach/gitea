...
// CellState is one release × environment cell of the matrix.
//
// Every state is a PROJECTION over the append-only audit log; none of them is stored, and
// no row carries an updatable status column the matrix reads. The `✔ ×N` rendering is the
...
type Cell struct {
	...
	SortOrder   int64     `json:"sort_order"` // the configured column order, so a matrix spanning repositories can merge columns
...
// MatrixRow is one release, with one cell per environment in configured order.
...
// deploy from a queued one. The projection answers it here so the matrix does not change
...
func projectOne(environment, release string, agg *aggregate, liveRelease, policy string) Cell {
	...
	case deployments_model.AuditRequested:
		...
		if policy != "" && policy != deployments_model.PolicyNone {
			cell.State = CellHeld
		}
	default:
...
// MatrixOptions narrows the matrix. The matrix spans the repositories the viewer can see, so
...
// BuildMatrix assembles the release × environment matrix.
//
...
func BuildMatrix(ctx context.Context, opts MatrixOptions) ([]*MatrixRow, int64, error) {
	if len(opts.RepoIDs) == 0 {
		return []*MatrixRow{}, 0, nil
	}
	...
	rows := make([]*MatrixRow, 0, len(releases))
	for _, release := range releases {
...
func environmentsOf(ctx context.Context, repoID int64, only string) ([]string, map[string]string, map[string]int64, error) {
	envs, _, err := deployments_model.FindEnvironments(ctx, builder.Eq{"repo_id": repoID}, "sort_order ASC, id ASC", 0, 0)
	if err != nil {
		return nil, nil, nil, err
	}
	if len(envs) == 0 {
		envs, _, err = deployments_model.FindEnvironments(ctx,
			builder.Eq{"repo_id": deployments_model.DefaultsRepoID}, "sort_order ASC, id ASC", 0, 0)
		if err != nil {
			return nil, nil, nil, err
		}
	}
	...
	for _, env := range envs {
		if only != "" && env.Name != deployments_model.NormalizeEnvironmentName(only) {
			continue
		}
...
// applySortOrders stamps each cell with its configured column order, which a matrix spanning repositories needs.
