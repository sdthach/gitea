...
func TestDeliveryCanApproveEnvironment(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			&deployments_model.Environment{Name: "prod"}, outsider, true, false, true,
		},
		{
			"block_admin_override drops the admin back to the dispatch set",
			&deployments_model.Environment{Name: "prod", BlockAdminOverride: true}, outsider, true, false, false,
		},
		{
			"block_admin_override still lets a dispatcher approve",
			&deployments_model.Environment{Name: "prod", BlockAdminOverride: true}, outsider, true, true, true,
		},
		{
			"an allowlisted user passes even without dispatch rights",
			&deployments_model.Environment{Name: "prod", EnableBypassAllowlist: true, BypassAllowlistUserIDs: []int64{outsider.ID}},
			outsider, false, false, true,
		},
		{
			"the allowlist refuses a dispatcher who is not on it",
			&deployments_model.Environment{Name: "prod", EnableBypassAllowlist: true, BypassAllowlistUserIDs: []int64{999}},
			outsider, false, true, false,
		},
		{
			"an allowlisted team's member passes",
			&deployments_model.Environment{Name: "prod", EnableBypassAllowlist: true, BypassAllowlistTeamIDs: []int64{1}},
		...
		},
		{
			...
			&deployments_model.Environment{Name: "prod", EnableBypassAllowlist: true},
...
func decideFixture(t *testing.T, policy string, required int64) (*deployments_model.Approval, *deployments_model.Environment) {
	t.Helper()
	require.NoError(t, unittest.PrepareTestDatabase())
	...
	hold := &deployments_model.Approval{
		RepoID: 1, Environment: "prod", RunID: 4242, JobID: 84, ReleaseTag: "v1.1",
		RequesterID: 2, RequesterLogin: "user2",
	}
	require.NoError(t, deployments_model.AppendApproval(t.Context(), hold))
	return hold, env
}
...
func TestDeliveryDecideWritesAnAuditEventAndReleases(t *testing.T) {
	...
	assert.Equal(t, deployments_model.ApprovalApproved, decision.State)
	assert.Equal(t, int64(1), decision.ApprovalsCount)
	assert.Equal(t, int64(1), decision.RequiredApprovals)

	votes, err := deployments_model.VotesForApproval(t.Context(), hold)
	require.NoError(t, err)
...
func TestDeliveryDecideRefusals(t *testing.T) {
	...
	t.Run("approving twice as the same user is refused", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, deployments_model.ApprovalPending, decision.State,
	...
	t.Run("a rejection ends the deploy and no later approval revives it", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, deployments_model.ApprovalRejected, decision.State)
