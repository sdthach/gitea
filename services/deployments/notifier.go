...
func EnvironmentFromWorkflowID(workflowID string) string {
	...
	for _, ext := range []string{".yaml", ".yml"} {
		if trimmed, found := strings.CutSuffix(name, ext); found {
			return deployments_model.NormalizeEnvironmentName(trimmed)
		}
	}
...
func EventForRunStatus(status actions_model.Status) string {
	...
	case actions_model.StatusWaiting, actions_model.StatusBlocked:
		return deployments_model.AuditRequested
	case actions_model.StatusRunning:
		return deployments_model.AuditStarted
	case actions_model.StatusSuccess:
		return deployments_model.AuditSucceeded
	case actions_model.StatusFailure:
		return deployments_model.AuditFailed
	case actions_model.StatusCancelled, actions_model.StatusSkipped:
		return deployments_model.AuditCancelled
	}
	return ""
}
...
func RecordsForRun(repo *repo_model.Repository, sender *user_model.User, run *actions_model.ActionRun) (*deployments_model.Deployment, *deployments_model.AuditEvent, bool) {
	if repo == nil || run == nil {
	...
	deployment := &deployments_model.Deployment{
		RepoID:      repo.ID,
	...
	audit := &deployments_model.AuditEvent{
		Event:        event,
...
// NewNotifier builds the fork's notifier. It is exported so services/hub's own Init can
// register it, and so a test can register it against a test engine without going through
// Init.
