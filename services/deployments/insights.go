...
// The cross-repository CI insights.
//
...
// RunState is a run's state as the insights group them. The five states the tiles
...
// RunStateOf maps Gitea's own run status onto the insights' state. It reads the upstream
...
// on the name the insights show rather than on Gitea's internal integer.
...
// DefaultWindowDays is the window the insights open on.
...
// runs, so this dashboard and the deployments matrix cannot disagree about how many deploys
// happened.
...
// InsightsOptions narrows an aggregate. RepoIDs is always the caller's own accessible set,
...
func (o InsightsOptions) scope() []int64 {
	if len(o.RepoIDs) == 0 {
		return nil
	}
	if o.RepoID == 0 {
...
// Insights is the composite: the window's summary beside the previous window of equal
// length. It exists to save round trips, never as the only way to reach the data — every
// number in it is independently queryable from /runs, /workflows and /insights/repos.
...
// BuildInsights assembles the composite. It is the endpoint's whole implementation: the
// /insights handler resolves permissions and the window, and calls this.
func BuildInsights(ctx context.Context, opts InsightsOptions) (*Insights, error) {
	repoIDs := opts.scope()
	if len(repoIDs) == 0 {
		empty := SummarizeRuns(nil, opts.Window, 0, nil)
		return &Insights{Summary: empty, Previous: SummarizeRuns(nil, opts.Window.Previous(), 0, nil)}, nil
	}
...
func BuildTrends(ctx context.Context, opts InsightsOptions) ([]TrendPoint, bool, error) {
	repoIDs := opts.scope()
	if len(repoIDs) == 0 {
...
func BuildRepoStats(ctx context.Context, opts InsightsOptions) ([]RepoStat, bool, error) {
	repoIDs := opts.scope()
	if len(repoIDs) == 0 {
...
func BuildWorkflowStats(ctx context.Context, opts InsightsOptions) ([]WorkflowStat, bool, error) {
	repoIDs := opts.scope()
	if len(repoIDs) == 0 {
...
// links to Gitea's own page; the insights duplicate none of them.
