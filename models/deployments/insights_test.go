...
func TestDeploymentsRunFactDurationIgnoresUnfinishedAndInvertedRuns(t *testing.T) {
...
// TestDeploymentsFindRunFactsIsScopedToTheGivenRepositories is the permission filter's data
...
func TestDeploymentsFindRunFactsIsScopedToTheGivenRepositories(t *testing.T) {
...
// TestDeploymentsFindRunFactsIsFailClosedOnAnEmptyScope is the branch a fail-OPEN filter would
// get wrong: "no accessible repository" must aggregate nothing, never everything.
func TestDeploymentsFindRunFactsIsFailClosedOnAnEmptyScope(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())
...
// TestDeploymentsFindRunFactsHonoursTheWindow proves the window is half-open, so a run on the
// boundary is counted by exactly one of a window and its predecessor rather than by both.
func TestDeploymentsFindRunFactsHonoursTheWindow(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())
	ctx := t.Context()
...
// TestDeploymentsFindDeploymentFactsReadsTheForksOwnTable: the daily trend's deployment count
// comes from deploy_deployment, not from counting runs.
func TestDeploymentsFindDeploymentFactsReadsTheForksOwnTable(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())
	ctx := t.Context()
...
// TestDeploymentsFindRunsAppliesTheGrammarsCondition covers the /runs read path: it takes the
// condition the one grammar rendered rather than growing a second filter implementation.
func TestDeploymentsFindRunsAppliesTheGrammarsCondition(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())
	ctx := t.Context()
...
// TestDeploymentsFindDisabledWorkflowsReadsGiteasOwnUnitConfig proves the "workflows disabled"
// tile counts what Gitea's own repository settings wrote, rather than a mirrored list the
// fork would have to keep in step.
func TestDeploymentsFindDisabledWorkflowsReadsGiteasOwnUnitConfig(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())
	ctx := t.Context()
