...
// gate function living in models/delivery could not be called from models/actions without
// an import cycle. Measured in this tree: `go list -f '{{.Imports}}' ./models/delivery`
// lists gitea.dev/models/actions. This package is what the spoke names instead, and
// models/delivery registers the real gate into it from its own init().
...
// Register installs the gate. models/delivery calls it from its own init(), so the gate is
