...
import (
	...
	"gitea.dev/models/db"
	"gitea.dev/models/delivery/approvalgate"
...
// CreateTaskForRunner reaches through models/delivery/approvalgate — not only the pure
...
// spoke work: with nothing registered the dispatcher answers false, and models/delivery's
...
func TestDeliveryApprovalGateSeamIsWired(t *testing.T) {
	...
	assert.True(t, approvalgate.Held(t.Context(), 7, 5),
		"models/delivery's init must have registered the gate, or the spoke calls nothing")
