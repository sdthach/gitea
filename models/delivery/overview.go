...
// one schema has to answer both (M3). Reducing each row to six columns first is what keeps
// that affordable.
...
// DeploymentFact is one deployment reduced to what the daily trend needs (P5).
...
// resolved by Gitea's existing permission filtering before it reaches here (P6, E12).
...
// CI overview and the delivery grid share one source of truth (P5).
...
// endpoint for (P8). It takes the condition the query grammar rendered, so filtering,
