...
var secretScopeBodyParams = []hubapi.Param{
...
var secretScopeIDParam = []hubapi.Param{
	{Name: "id", In: "path", Type: "integer", Description: "Secret scope id.", Required: true},
}

func createSecretScopeEndpoint() *hubapi.Endpoint {
...
func deleteSecretScopeEndpoint() *hubapi.Endpoint {
	return &hubapi.Endpoint{
		Op: &hubapi.Operation{
...
func CreateSecretScopeHandler(ctx *context.APIContext) {
	...
	scope := &deployments_model.SecretScope{
		RepoID:      body.RepoID,
		SecretName:  body.SecretName,
		Environment: body.Environment,
	}
	if err := deployments_model.BindSecretScope(ctx, scope); err != nil {
...
func DeleteSecretScopeHandler(ctx *context.APIContext) {
	...
	existing, err := deployments_model.GetSecretScopeByID(ctx, id)
	if err != nil {
	...
	if err := deployments_model.UnbindSecretScope(ctx, id); err != nil {
		hubapi.RenderHubError(ctx, http.StatusNotFound, err)
		return
	}
