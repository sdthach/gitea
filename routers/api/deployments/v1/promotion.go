...
var promotionBodyParams = []hubapi.Param{
...
func createDeploymentEndpoint() *hubapi.Endpoint {
...
func CreateDeployment(ctx *context.APIContext) {
	...
	result, err := deployments_service.Promote(ctx, deployments_service.PromotionRequest{
		Repo:           repo,
		Doer:           ctx.Doer,
	...
	if err != nil {
		...
		if notFound, ok := errors.AsType[*deployments_service.ErrPromotionNotFound](err); ok {
			status, err = http.StatusNotFound, notFound.Err
		}
		hubapi.RenderHubError(ctx, status, err)
		return
	}

	switch {
	case result.Outcome == deployments_service.OutcomeRefuse:
