...
var componentSchemas = map[string]any{
	"Environment": hubapi.ObjectSchema(map[string]any{
		...
		"review_policy":            hubapi.EnumProp("Review policy. Defaults to none, so adding the fork changes no existing behaviour.", deployments_model.ReviewPolicies),
		"required_reviewers":       hubapi.Prop("integer", "Reviews a held deploy needs. Defaults to 1."),
		...
		"auto_promote":             hubapi.Prop("boolean", "Deploy the same release here automatically once every environment in depends_on holds it live."),
		"wait_minutes":             hubapi.Prop("integer", "Hold every deploy this many minutes after it is requested before dispatching. 0..10080 (a week)."),
		"deploy_window":            hubapi.Prop("object", "The recurring time-of-day window a deploy must land inside: days_mask, from_minute, to_minute, timezone. Null means always open."),
		"required_status_contexts": hubapi.ArrayProp("string", "Commit status contexts that must report success on the release commit before dispatching."),
		"exclusive_lock":           hubapi.Prop("boolean", "Refuse a deploy while another is already waiting or running on this environment."),
	}, "id", "repo_id", "name", "sort_order", "review_policy", "required_reviewers", "can_write"),
	"Check": hubapi.ObjectSchema(map[string]any{
		"name":             hubapi.EnumProp("Which pre-deployment check this is.", []string{"reviewers", "prior_deployment", "releases_only", "wait_timer", "deployment_window", "required_status_contexts", "exclusive_lock"}),
		"state":            hubapi.EnumProp("The check's answer.", []string{"pass", "wait", "fail"}),
		"reason":           hubapi.Prop("string", "Why, when the state is not pass."),
		"suggested_action": hubapi.Prop("string", "What to do about it."),
		"retry_at":         hubapi.Prop("integer", "Unix seconds worth retrying at, when the check names one."),
	}, "name", "state"),
	"PromotionPath": hubapi.ObjectSchema(map[string]any{
		"nodes": hubapi.ArrayProp("object", "One entry per environment: name, depends_on, auto_promote and checks (wait_minutes, deploy_window, required_status_contexts, exclusive_lock)."),
		"edges": hubapi.ArrayProp("object", "One entry per dependency: from (holds the release first) and to."),
	}, "nodes", "edges"),
	...
	"Promotion": hubapi.ObjectSchema(map[string]any{
		...
		"depends_on":               hubapi.ArrayProp("string", "The environments this one depends on; empty when it depends on none."),
		"sha":                      hubapi.Prop("string", "The release's commit, used to evaluate required_status_contexts."),
		"predecessor_state":        hubapi.EnumProp("What the first unsatisfied dependency has done with this release: none, never, held or live.", []string{"none", "never", "held", "live"}),
		...
		"deployment_id":            hubapi.Prop("integer", "The deploy_deployment row's own id, set only when this call appended a waiting placeholder. GET /deployments/{id}/checks takes this id."),
		"state":                    hubapi.EnumProp("This call's own outcome.", []string{"planned", "refused", "override_required", "checks_failed", "waiting", "dispatched"}),
		"checks":                   hubapi.ArrayProp("object", "Every pre-deployment check EvaluateChecks ran, in order."),
	}, "repo_id", "repo_full_name", "environment", "release_tag", "outcome", "predecessor_state", "confirmed", "state"),
