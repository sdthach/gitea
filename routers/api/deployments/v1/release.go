...
type Release struct {
	...
	Deployments  []*deployments_model.Deployment `json:"deployments,omitempty"`
}
...
func listReleasesEndpoint() *hubapi.Endpoint {
...
func ListReleases(ctx *context.APIContext) {
	...
	q, ok := hubapi.ParseQuery(ctx, releaseSpec)
	if !ok {
		return
	}
...
func expandReleases(ctx *context.APIContext, expand []string, repoID int64, rows []*Release) error {
	for _, name := range expand {
		...
		for _, row := range rows {
			...
			deployments, err := deployments_model.FindDeployments(ctx, cond, "created_unix ASC, id ASC", query.MaxLimit)
			if err != nil {
				return err
			}
