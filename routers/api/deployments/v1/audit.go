...
func listAuditEndpoint() *hubapi.Endpoint {
...
func ListAudit(ctx *context.APIContext) {
	q, ok := hubapi.ParseCursorQuery(ctx, auditSpec)
	if !ok {
		return
	}
	...
	if len(repoIDs) == 0 {
		hubapi.RenderCursorPage(ctx, q, 0, nil, 0, []*deployments_model.AuditEvent{})
		return
	}

	cond := q.Cond().And(builder.In("repo_id", repoIDs)).And(q.CursorCond())
	rows, err := deployments_model.FindAuditEvents(ctx, cond, q.OrderBy(), q.Limit)
	if err != nil {
	...
	hubapi.RenderCursorPage(ctx, q, len(rows), sortValue, lastID, rows)
}

// auditSortValue reads the column the traversal is sorted on out of a row.
func auditSortValue(column string, e *deployments_model.AuditEvent) any {
	switch column {
	case "id":
		return e.ID
	case "event":
