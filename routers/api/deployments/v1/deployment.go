...
func ListDeployments(ctx *context.APIContext) {
	q, ok := hubapi.ParseCursorQuery(ctx, deploymentSpec)
	if !ok {
		return
	}
	...
	if len(repoIDs) == 0 {
		hubapi.RenderCursorPage(ctx, q, 0, nil, 0, []*Deployment{})
		return
	}
...
func deploymentSortValue(column string, d *deployments_model.Deployment) any {
	switch column {
	case "id":
		return d.ID
	case "environment":
