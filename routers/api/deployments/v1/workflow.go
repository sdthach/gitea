...
func workflowStatValue(row deployments_service.WorkflowStat, field string) (any, bool) {
	switch field {
	case "repo_id":
...
func listWorkflowsEndpoint() *hubapi.Endpoint {
...
func ListWorkflows(ctx *context.APIContext) {
	q, ok := hubapi.ParseQuery(ctx, workflowSpec)
	if !ok {
		return
	}
	...
	rows, _, err := deployments_service.BuildWorkflowStats(ctx, opts)
	if err != nil {
		ctx.APIErrorInternal(err)
		return
	}
	rows = filterProjection(rows, q, workflowStatValue)
	deployments_service.SortWorkflowStats(rows, q.Sort.Column, q.Sort.Order)
