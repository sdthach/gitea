...
var ownerRepoParams = []hubapi.Param{
...
func listEnvironmentsEndpoint() *hubapi.Endpoint {
...
func listRepoEnvironmentsEndpoint() *hubapi.Endpoint {
	return &hubapi.Endpoint{
		Op: &hubapi.Operation{
...
func getEnvironmentEndpoint() *hubapi.Endpoint {
	return &hubapi.Endpoint{
		Op: &hubapi.Operation{
...
func getRepoEnvironmentEndpoint() *hubapi.Endpoint {
	return &hubapi.Endpoint{
		Op: &hubapi.Operation{
...
type EnvironmentRow struct {
	*deployments_model.Environment
	CanWrite bool `json:"can_write"`
}

func environmentRows(ctx *context.APIContext, envs []*deployments_model.Environment) ([]*EnvironmentRow, error) {
...
func renderEnvironments(ctx *context.APIContext, q *query.Query, total int64, envs []*deployments_model.Environment) {
...
func renderEnvironment(ctx *context.APIContext, status int, env *deployments_model.Environment) {
	rows, err := environmentRows(ctx, []*deployments_model.Environment{env})
	if err != nil {
		ctx.APIErrorInternal(err)
		return
	}
...
func ListEnvironments(ctx *context.APIContext) {
	q, ok := hubapi.ParseQuery(ctx, environmentSpec)
	if !ok {
		return
	}
	...
	visible := builder.Eq{"repo_id": deployments_model.DefaultsRepoID}.Or(builder.In("repo_id", repoIDs))
	envs, total, err := deployments_model.FindEnvironments(ctx, q.Cond().And(visible), q.OrderBy(), q.Limit, q.Offset())
	if err != nil {
		ctx.APIErrorInternal(err)
		return
	}
...
func GetEnvironment(ctx *context.APIContext) {
	...
	env, err := deployments_model.GetEnvironmentByID(ctx, id)
	...
	if !visible { // one refusal for a hidden row and a missing one, so a 404 confirms nothing
		hubapi.APIError(ctx, http.StatusNotFound, "environment_not_found",
...
func ListRepoEnvironments(ctx *context.APIContext) {
	...
	envs, total, err := deployments_model.FindEnvironments(ctx, q.Cond().And(scope), q.OrderBy(), q.Limit, q.Offset())
	if err != nil {
		ctx.APIErrorInternal(err)
		return
	}
	if total == 0 && len(q.Filters) == 0 && q.Search == "" {
		defaults := builder.Eq{"repo_id": deployments_model.DefaultsRepoID}
		envs, total, err = deployments_model.FindEnvironments(ctx, defaults, q.OrderBy(), q.Limit, q.Offset())
		if err != nil {
			ctx.APIErrorInternal(err)
			return
		}
