...
func listRunsEndpoint() *hubapi.Endpoint {
...
func ListRuns(ctx *context.APIContext) {
	q, ok := hubapi.ParseQuery(ctx, runSpec)
	if !ok {
		return
	}
	if qErr := mapRunStatusFilters(q); qErr != nil {
		hubapi.RenderQueryError(ctx, qErr)
		return
	}
	...
	if len(repoIDs) == 0 {
		hubapi.RenderPage(ctx, q, 0, []*Run{})
		return
	}

	cond := q.Cond().And(builder.In("repo_id", repoIDs))
	runs, total, err := deployments_model.FindRuns(ctx, cond, q.OrderBy(), q.Limit, q.Offset())
	if err != nil {
...
func mapRunStatusFilters(q *query.Query) *query.Error {
	for i := range q.Filters {
		...
		for _, raw := range f.Values {
			...
			mapped, known := deployments_service.RunStatusCodes(name)
			if !known {
				return &query.Error{
					...
					Accepted:        deployments_service.RunStateNames(),
					SuggestedAction: "Filter on one of the accepted states: " + strings.Join(deployments_service.RunStateNames(), ", ") + ".",
				}
			}
...
func convertRuns(ctx *context.APIContext, runs []*actions_model.ActionRun) ([]*Run, error) {
	...
	for _, r := range runs {
		row := &Run{
			...
			Status:      string(deployments_service.RunStateOf(int(r.Status))),
			CreatedUnix: int64(r.Created),
			StartedUnix: int64(r.Started),
			StoppedUnix: int64(r.Stopped),
		}
		fact := deployments_model.RunFact{StartedUnix: row.StartedUnix, StoppedUnix: row.StoppedUnix}
