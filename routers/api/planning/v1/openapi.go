...
var componentSchemas = map[string]any{
	"Board": hubapi.ObjectSchema(map[string]any{
		...
		"group_by":       hubapi.EnumProp("The active lane grouping. A view setting, never stored on the project.", planning_service.Groupings),
		...
		"lanes": hubapi.ArrayProp("object", "The horizontal lanes Gitea does not model. Each carries key, label, is_empty_value, "+
		...
		"can_edit_issue": hubapi.Prop("boolean", "Whether the caller may move a card between lanes, which edits the issue's own label or assignee."),
	}, "repo_id", "repo_full_name", "project_id", "group_by", "columns", "lanes", "can_write", "can_edit_issue"),
	"Timeline": hubapi.ObjectSchema(map[string]any{
		...
		"spans": hubapi.ArrayProp("object", "Epic and milestone rows spanning earliest start to latest end of their children, with "+
		...
		"group_by": hubapi.EnumProp("The active lane grouping, reusing the board's own. A view setting, never stored.", planning_service.Groupings),
		...
		"lanes": hubapi.ArrayProp("object", "The PUBLISHED bars grouped by the board's own lane definition. Empty when grouping is off, "+
