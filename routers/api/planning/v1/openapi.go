...
var componentSchemas = map[string]any{
	"Board": hubapi.ObjectSchema(map[string]any{
		...
		"types": hubapi.ArrayProp("object", "The types visible from this repository, nearest scope shadowing by name — what a card's "+
			...
			"milestone_id, parent_issue_id, root_issue_id, depth, has_children, time_estimate, tracked_seconds and "+
			"assignee_avatars (login, avatar_url — one per entry in assignees, in the same order, resolved through "+
			"the user's own avatar link so gravatar and local avatar settings are honoured)."),
	...
	"Roadmap": hubapi.ObjectSchema(map[string]any{
		...
		"bars": hubapi.ArrayProp("object", "One bar per managed issue, empty at a rolled-up zoom. Each carries issue_id, number, "+
			"title, url, type, type_id, type_color, type_icon, labels, assignees, assignee_avatars, milestone, start_unix, "+
			"end_unix, start_source, end_source, end_inferred, is_closed, parent_issue_id, root_issue_id, depth, has_children, "+
			"time_estimate and tracked_seconds. assignee_avatars pairs each entry of assignees, in the same order, with the "+
			"avatar url resolved through that user's own avatar link, so gravatar and local avatar settings are honoured. "+
		...
		"unmanaged": hubapi.ArrayProp("object", "Issues with no bar, each with the reason and a suggested action. An issue with no "+
			...
			"also carries labels, assignees, assignee_avatars, type, type_id, milestone_id, is_closed, time_estimate and tracked_seconds, the same fields a bar would have."),
