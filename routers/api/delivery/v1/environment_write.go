...
var environmentBodyParams = []Param{
...
func updateEnvironmentEndpoint() *endpoint {
	return &endpoint{
		Op: &Operation{
...
func deleteEnvironmentEndpoint() *endpoint {
	return &endpoint{
		Op: &Operation{
...
func CreateEnvironmentHandler(ctx *context.APIContext) {
	...
	if err := delivery.CreateEnvironment(ctx, env); err != nil {
...
func UpdateEnvironmentHandler(ctx *context.APIContext) {
	...
	existing, err := delivery.GetEnvironmentByID(ctx, id)
	if err != nil {
	...
	if err := delivery.UpdateEnvironment(ctx, env); err != nil {
		renderHubError(ctx, http.StatusBadRequest, err)
		return
	}
...
func DeleteEnvironmentHandler(ctx *context.APIContext) {
	...
	if err := delivery.DeleteEnvironment(ctx, id); err != nil {
		renderHubError(ctx, http.StatusNotFound, err)
		return
	}
...
func bodyToEnvironment(body *environmentBody) *delivery.Environment {
