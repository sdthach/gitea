...
func withRecorder(t *testing.T, rec *recorder) {
	...
	t.Setenv("GITEA_DEPLOYMENTS_SERVER", "https://gitea.example.invalid")
	t.Setenv("GITEA_DEPLOYMENTS_TOKEN", "t0ken")
...
func lookup(name string) (hubcli.Command, bool) {
	for _, c := range Commands {
		if c.Name == name {
			return c, true
		}
	}
	return hubcli.Command{}, false
}
...
func TestEveryCommandComposesItsRequest(t *testing.T) {
	cases := map[string]struct {
		...
		"environments":             {[]string{"environments"}, "/api/deployments/v1/environments", ""},
		"repos":                    {[]string{"repos"}, "/api/deployments/v1/repos", ""},
		"repo-environments":        {[]string{"repo-environments", "acme", "web"}, "/api/deployments/v1/repos/acme/web/environments", ""},
		"repo-environment":         {[]string{"repo-environment", "acme", "web", "prod"}, "/api/deployments/v1/repos/acme/web/environments/prod", ""},
		"repo-environment-secrets": {[]string{"repo-environment-secrets", "acme", "web", "prod"}, "/api/deployments/v1/repos/acme/web/environments/prod/secrets", ""},
		...
		"environment-update": {
			...
			"/api/deployments/v1/environments/7", http.MethodPut,
		},
		"environment-delete": {
			[]string{"environment-delete", "7"},
			"/api/deployments/v1/environments/7", http.MethodDelete,
		},
		"secret-scope-create": {
			[]string{"secret-scope-create", "--repo-id", "1", "--secret-name", "DEPLOY_KEY", "--environment", "prod"},
			"/api/deployments/v1/secret-scopes", http.MethodPost,
		},
		"secret-scope-delete": {
...
func TestMissingCredentialsNameWhatToSet(t *testing.T) {
	previous := hubcli.Transport
	hubcli.Transport = &recorder{body: "[]"}
	t.Cleanup(func() { hubcli.Transport = previous })
	for _, name := range append(append([]string{}, config.TokenEnvVars...), config.ServerEnvVars...) {
		t.Setenv(name, "")
	}
...
func TestCredentialPrecedence(t *testing.T) {
	...
	cases := []struct {
		...
		{"flag wins", "flagged", map[string]string{"GITEA_DEPLOYMENTS_TOKEN": "a"}, "flagged", "--token"},
		{"deployments variable", "", map[string]string{"GITEA_DEPLOYMENTS_TOKEN": "a", "FORGE_TOKEN": "b"}, "a", "$GITEA_DEPLOYMENTS_TOKEN"},
		...
		{"empty is not set", "", map[string]string{"GITEA_DEPLOYMENTS_TOKEN": "", "GITEA_TOKEN": "c"}, "c", "$GITEA_TOKEN"},
		{"nothing", "", nil, "", ""},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			value, source := hubcli.ResolveToken(c.flag, config.TokenEnvVars, env(c.env))
...
func TestResolveServerTrimsTrailingSlash(t *testing.T) {
	value, source := hubcli.ResolveServer("https://gitea.example.invalid/", config.ServerEnvVars, func(string) (string, bool) { return "", false })
...
func TestListOperationsIsStable(t *testing.T) {
	...
	require.Nil(t, hubcli.Run(config, []string{"--list-operations"}, &stdout, io.Discard))
...
func TestUsageListsEveryCommand(t *testing.T) {
	for _, args := range [][]string{{}, {"--help"}, {"-h"}, {"help"}} {
		var stdout bytes.Buffer
		require.Nil(t, hubcli.Run(config, args, &stdout, io.Discard))
		body := stdout.String()
		...
		for _, source := range config.TokenEnvVars {
...
func TestSubcommandHelpGoesToStdoutAndExitsZero(t *testing.T) {
	for _, cmd := range Commands {
		t.Run(cmd.Name, func(t *testing.T) {
			var stdout, stderr bytes.Buffer
			require.Nil(t, hubcli.Run(config, []string{cmd.Name, "--help"}, &stdout, &stderr))
...
func TestDeploymentsRunsFilterComposesTheFailedRunsRequest(t *testing.T) {
	...
	assert.Equal(t, "/api/deployments/v1/runs", req.URL.Path)
