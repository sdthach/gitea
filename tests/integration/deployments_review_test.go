...
// TestDeploymentsReviewGateHoldsJobsUntilApproved runs the gate through the
...
func TestDeploymentsReviewGateHoldsJobsUntilApproved(t *testing.T) {
...
// TestDeploymentsReviewRejectionEndsTheDeploy is the other half: a rejection is terminal,
// the run does not proceed later, and the rejection names its approver on the audit log.
func TestDeploymentsReviewRejectionEndsTheDeploy(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, _ *url.URL) {
		repo, sha := commitWorkflow(t)
		ctx := t.Context()
...
// TestDeploymentsReviewsPageIsAClientOfTheAPI covers the pending-review view: the
...
func TestDeploymentsReviewsPageIsAClientOfTheAPI(t *testing.T) {
	...
	assert.Contains(t, resp.Body.String(), `"prod"`,
		"the environment's own pending list is filtered to it, so an approver need not hunt the matrix")
}

// TestDeploymentsReviewGateLeavesUngatedRepositoriesAlone is the "with the fork absent"
...
func TestDeploymentsReviewGateLeavesUngatedRepositoriesAlone(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, _ *url.URL) {
		repo, sha := commitWorkflow(t)
		ctx := t.Context()
