...
// TestDeploymentsDeployPostsACommitStatusAndMovesTheDeployedTag is what makes a deploy visible
...
func TestDeploymentsDeployPostsACommitStatusAndMovesTheDeployedTag(t *testing.T) {
...
// TestDeploymentsFailedDeployPostsAFailureAndLeavesTheTagAlone is the other half: a failure is
// reported where a success would be, and nothing claims the commit is deployed.
func TestDeploymentsFailedDeployPostsAFailureAndLeavesTheTagAlone(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, _ *url.URL) {
		user := unittest.AssertExistsAndLoadBean(t, &user_model.User{ID: 2})
		repo := unittest.AssertExistsAndLoadBean(t, &repo_model.Repository{ID: 1, OwnerID: user.ID})
